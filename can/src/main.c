...
STATIC RINGBUFF_T rx_buffer; //used globally
CCAN_MSG_OBJ_T _rx_buffer[8]; //underscore something = used internally
...
static void LED_Init(uint8_t port, uint8_t pin) {
	...
	Chip_GPIO_SetPinState(LPC_GPIO, port, pin, false); //turnoff
...
void CAN_rx(uint8_t msg_obj_num) {
    ...
    uint8_t *response_msg = "got messasge 1";
	if (msg_obj_num == 1) {
...
void CAN_tx(uint8_t msg_obj_num) {
    uint8_t *transmit_msg = "message transmitted";
